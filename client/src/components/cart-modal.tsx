...
export default function CartModal() {
  ...
  const handleQuickOrder = () => {
    ...
    const orderSummary = state.items.map(item => 
      `${item.quantity}x ${item.menuItem.name} - DOP $${(parseFloat(item.menuItem.price) * item.quantity)}`
    ).join('\n');
...
💰 *Subtotal: DOP $${getSubtotal()}*
🚚 *Delivery Fee: ${deliveryFee === 0 ? 'FREE' : 'DOP $' + deliveryFee}*
💰 *Total: DOP $${getFinalTotal()}*
                ...
                {state.items.map((item) => (
                  <div key={item.menuItem.id} className="flex items-center space-x-3 glass-card rounded-2xl p-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">DOP ${parseFloat(item.menuItem.price)}</p>
                    </div>
              ...
              <div className="space-y-3">
                ...
                <Select value={deliveryLocation} onValueChange={setDeliveryLocation}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="cabarete">Cabarete (DOP $100)</SelectItem>
                    <SelectItem value="sosua">Sosua (DOP $150)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={getDeliveryFee() === 0 ? "text-green-600 font-semibold" : ""}>
                    {getDeliveryFee() === 0 ? "FREE" : `DOP $${getDeliveryFee()}`}
                  </span>
                </div>
                <Separator />
